...
run_sync() {
  ...
  #
  # dotglob is enabled locally so hidden entries (names beginning with ".") are
  # enumerated too — otherwise a dangling symlink like ".foo" would be invisible
  # to this glob and could never be pruned (Windows uses Get-ChildItem -Force,
  # which already sees hidden entries). .git and friends remain excluded below.
  local dangling_removed=0
  declare -a external_link_skills=()
  local _prev_dotglob _prev_nullglob
  _prev_dotglob=$(shopt -p dotglob 2>/dev/null || true)
  _prev_nullglob=$(shopt -p nullglob 2>/dev/null || true)
  shopt -s dotglob nullglob
  for _entry in "$CENTRAL_DIR"/*; do
    [ -L "$_entry" ] || continue
    _e_name=$(basename "$_entry")
    [[ "$_e_name" == node_modules || "$_e_name" == .git || "$_e_name" == dist || "$_e_name" == docs || "$_e_name" == instructions || "$_e_name" == mcp || "$_e_name" == _* || "$_e_name" == EasySkills维护工具 || "$_e_name" == .maintenance-* || "$_e_name" == .runtime || "$_e_name" == EasySkills维护工具.* ]] && continue
    if [ ! -e "$_entry" ]; then
      unlink "$_entry"
  ...
  eval "$_prev_dotglob" 2>/dev/null || shopt -u dotglob
  eval "$_prev_nullglob" 2>/dev/null || shopt -u nullglob
  unset _entry _e_name _prev_dotglob _prev_nullglob
  ...
  for skill_dir in "$CENTRAL_DIR"/*/; do
    ...
    [[ "$skill_name" == "node_modules" || "$skill_name" == ".git" || "$skill_name" == "dist" || "$skill_name" == "docs" || "$skill_name" == "instructions" || "$skill_name" == "mcp" || "$skill_name" == "." || "$skill_name" == ".." || "$skill_name" == _* || "$skill_name" == "EasySkills维护工具" || "$skill_name" == .maintenance-* || "$skill_name" == ".runtime" || "$skill_name" == EasySkills维护工具.* ]] && continue
