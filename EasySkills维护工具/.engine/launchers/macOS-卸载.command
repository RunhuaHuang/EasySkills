...
for script_name in webui-service.sh webui.py; do
  ...
  if [ -n "$pids" ]; then
    echo "$pids" | while read -r p; do
      ...
      base="${comm##*/}"
      [[ "$cmdline" == *"$INSTALL_DIR/EasySkills维护工具/.engine/$script_name"* ]] || continue
...
if [ -d "$INSTALL_DIR/EasySkills维护工具/.engine" ]; then
  if ! bash "$INSTALL_DIR/EasySkills维护工具/.engine/deploy.sh" --cleanup; then
