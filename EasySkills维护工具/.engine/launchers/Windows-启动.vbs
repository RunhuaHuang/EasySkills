...
' NOTE: maintDir contains the Unicode engine name (e.g. EasySkills维护工具/.engine).
' WSH is Unicode (UTF-16) throughout, so building these paths dynamically from
' the script's own parent folder is safe on every Windows codepage/locale —
' no literal path strings are used here.
